...
test.describe('Test Isolation', () => {
  ...
  test('Test 4: localStorage is cleared between tests', async ({ page }) => {
    // Navigate first — localStorage requires a page origin
  ...
  test('Test 5: Global state is cleared', async ({ page }) => {
    // Must start game first so gs is defined and initialised
    await startNewGame(page, { dynasty: 'MutateTest' });

    // Mutate gs — assign to the let binding directly, not window.gs
    await page.evaluate(() => {
      Object.assign(gs, { marks: 9999 });
    });
